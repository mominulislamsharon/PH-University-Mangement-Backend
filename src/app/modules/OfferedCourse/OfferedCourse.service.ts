...
const createOfferedCourseIntoDB = async (payload: TOfferedCourse) => {
  ...
  // if the semester registration idis exists!
  ...
  if (!isDepartmentBelongToFaculty) {
    throw new AppError(
      ...
      `This ${isAcademicDepartmentExists} is not  belong to this ${isAcademicFacultyExists}`,
    );
  }

  // check if the same offered course same section in same registered semsester exists
...
const getAllOfferedCourseFromDB = async (query: Record<string, unknown>) => {
  const offeredCouresQuery = new QueryBuilder(OfferedCourse.find(), query)
    .filter()
    .sort()
    .paginate()
    .fields();

  const result = await offeredCouresQuery.modelQuery;

  const meta = await offeredCouresQuery.countTotal();
