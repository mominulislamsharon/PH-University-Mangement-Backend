...
const createEnrolledCourseIntoDB = async (
  ...
  const isStudentAlreadyEnorolled = await EnrolledCourse.findOne({
    semesterRegistration: isOfferedCourseExists?.semesterRegistration,
    offeredCourse,
    student: student._id,
  });

  if (isStudentAlreadyEnorolled) {
  ...
  try {
    ...
    // payload.semesterRegistration = offeredCourse.isOfferedCourseExists
...
const updateEnrolledCourseMarksIntoDB = async (
  ...
  const isSemesterRegistrationCourseExists =
    await SemesterRegistration.findById(semesterRegistration);

  if (!isSemesterRegistrationCourseExists) {
